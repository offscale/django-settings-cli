...
class TestParser(TestCase):
    ...
    def test_empty_query(self):
        ...
        self.local_py_example_sio.seek(0)
        self.assertEqual(query_py_parser(infile=self.local_py_example_sio, query='.'), s)

    def test_simple_key_val(self):
        self.assertFalse(query_py_parser(infile=self.local_py_example_sio, query='.DEBUG'))

    def test_simple_key_collection_val(self):
        self.assertEqual(query_py_parser(infile=self.local_py_example_sio, query='.ADMINS'),
                         (('Admin', 'example@example.com'),))

    def test_sliced_key_collection_val(self):
        self.assertEqual(query_py_parser(infile=self.local_py_example_sio, query='.REST_FRAMEWORK.DEFAULT_THROTTLE_RATES'),
                         {'anon-read': None,
                          'anon-write': '20/min',
    ...
    def test_sliced_key_collection_val_sub_val(self):
        self.assertEqual(
            query_py_parser(infile=self.local_py_example_sio, query='.REST_FRAMEWORK.DEFAULT_THROTTLE_RATES.anon-write'),
            '20/min')

    def test_format_str(self):
        self.assertEqual(
            query_py_parser(
                infile=self.local_py_example_sio, query='.DATABASES.default',
